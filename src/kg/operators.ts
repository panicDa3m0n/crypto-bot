...
export class SwapOp implements Transformation {
  ...
  apply(s: SimulationState): ApplyResult {
    ...
    if (!o.exact) { s.exact = false; if (!s.inexactReason) s.inexactReason = o.inexactReason; } // Item 3: a non-certified leg makes the whole plan non-certifiable (still an economicCandidate)
