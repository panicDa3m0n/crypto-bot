...
export class StableAdapter implements VenueAdapter {
  ...
  quoteOut(pool: PoolState, tokenIn: string, amountIn: bigint): QuoteLeg | null {
    ...
    if (pool.dec0 == null || pool.dec1 == null || pool.feePpm <= 0) return null; // FAIL-CLOSED: decimals + real fee required, never guess
    const inl = tokenIn.toLowerCase();
    const inIs0 = inl === pool.token0.toLowerCase();
    if (!inIs0 && inl !== pool.token1.toLowerCase()) return null;
    const out = stableGetAmountOut(amountIn, inIs0, pool.r0, pool.r1, pool.dec0, pool.dec1, pool.feePpm / 100);
    return out > 0n ? { amountOut: out, feePpm: pool.feePpm, approximate: false } : null;
  }

  spot(pool: PoolState, base: string, _dec0: number, _dec1: number): number | null {
    if (pool.r0 == null || pool.r1 == null || pool.dec0 == null || pool.dec1 == null || pool.r0 <= 0n || pool.r1 <= 0n || pool.feePpm <= 0) return null;
    ...
    const out = stableGetAmountOut(probe, baseIs0, pool.r0, pool.r1, pool.dec0, pool.dec1, pool.feePpm / 100);
